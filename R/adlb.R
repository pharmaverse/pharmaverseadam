#' Dataset adlb
#'
#' adlb dataset
#'
#' @name adlb
#' @docType data
#' @format A data frame with 111 columns:
#'   \describe{
#'     \item{ STUDYID }{STUDYID}
#'     \item{ DOMAIN }{DOMAIN}
#'     \item{ USUBJID }{USUBJID}
#'     \item{ LBSEQ }{LBSEQ}
#'     \item{ LBTESTCD }{LBTESTCD}
#'     \item{ LBTEST }{LBTEST}
#'     \item{ LBCAT }{LBCAT}
#'     \item{ LBORRES }{LBORRES}
#'     \item{ LBORRESU }{LBORRESU}
#'     \item{ LBORNRLO }{LBORNRLO}
#'     \item{ LBORNRHI }{LBORNRHI}
#'     \item{ LBSTRESC }{LBSTRESC}
#'     \item{ LBSTRESN }{LBSTRESN}
#'     \item{ LBSTRESU }{LBSTRESU}
#'     \item{ LBSTNRLO }{LBSTNRLO}
#'     \item{ LBSTNRHI }{LBSTNRHI}
#'     \item{ LBNRIND }{LBNRIND}
#'     \item{ LBBLFL }{LBBLFL}
#'     \item{ VISITNUM }{VISITNUM}
#'     \item{ VISIT }{VISIT}
#'     \item{ VISITDY }{VISITDY}
#'     \item{ LBDTC }{LBDTC}
#'     \item{ LBDY }{LBDY}
#'     \item{ TRTSDT }{TRTSDT}
#'     \item{ TRTEDT }{TRTEDT}
#'     \item{ TRT01A }{TRT01A}
#'     \item{ TRT01P }{TRT01P}
#'     \item{ ADT }{ADT}
#'     \item{ ADY }{ADY}
#'     \item{ PARAMCD }{PARAMCD}
#'     \item{ PARAM }{PARAM}
#'     \item{ PARAMN }{PARAMN}
#'     \item{ PARCAT1 }{PARCAT1}
#'     \item{ AVAL }{AVAL}
#'     \item{ AVALC }{AVALC}
#'     \item{ ANRLO }{ANRLO}
#'     \item{ ANRHI }{ANRHI}
#'     \item{ DTYPE }{DTYPE}
#'     \item{ AVISIT }{AVISIT}
#'     \item{ AVISITN }{AVISITN}
#'     \item{ ONTRTFL }{ONTRTFL}
#'     \item{ ANRIND }{ANRIND}
#'     \item{ BASETYPE }{BASETYPE}
#'     \item{ ABLFL }{ABLFL}
#'     \item{ BASE }{BASE}
#'     \item{ BASEC }{BASEC}
#'     \item{ BNRIND }{BNRIND}
#'     \item{ CHG }{CHG}
#'     \item{ PCHG }{PCHG}
#'     \item{ ATOXDSCL }{ATOXDSCL}
#'     \item{ ATOXDSCH }{ATOXDSCH}
#'     \item{ ATOXGRL }{ATOXGRL}
#'     \item{ ATOXGRH }{ATOXGRH}
#'     \item{ ATOXGR }{ATOXGR}
#'     \item{ BTOXGRL }{BTOXGRL}
#'     \item{ BTOXGRH }{BTOXGRH}
#'     \item{ BTOXGR }{BTOXGR}
#'     \item{ R2BASE }{R2BASE}
#'     \item{ R2ANRLO }{R2ANRLO}
#'     \item{ R2ANRHI }{R2ANRHI}
#'     \item{ SHIFT1 }{SHIFT1}
#'     \item{ SHIFT2 }{SHIFT2}
#'     \item{ ANL01FL }{ANL01FL}
#'     \item{ LVOTFL }{LVOTFL}
#'     \item{ TRTP }{TRTP}
#'     \item{ TRTA }{TRTA}
#'     \item{ ASEQ }{ASEQ}
#'     \item{ SUBJID }{SUBJID}
#'     \item{ RFSTDTC }{RFSTDTC}
#'     \item{ RFENDTC }{RFENDTC}
#'     \item{ RFXSTDTC }{RFXSTDTC}
#'     \item{ RFXENDTC }{RFXENDTC}
#'     \item{ RFICDTC }{RFICDTC}
#'     \item{ RFPENDTC }{RFPENDTC}
#'     \item{ DTHDTC }{DTHDTC}
#'     \item{ DTHFL }{DTHFL}
#'     \item{ SITEID }{SITEID}
#'     \item{ AGE }{AGE}
#'     \item{ AGEU }{AGEU}
#'     \item{ SEX }{SEX}
#'     \item{ RACE }{RACE}
#'     \item{ ETHNIC }{ETHNIC}
#'     \item{ ARMCD }{ARMCD}
#'     \item{ ARM }{ARM}
#'     \item{ ACTARMCD }{ACTARMCD}
#'     \item{ ACTARM }{ACTARM}
#'     \item{ COUNTRY }{COUNTRY}
#'     \item{ DMDTC }{DMDTC}
#'     \item{ DMDY }{DMDY}
#'     \item{ TRTSDTM }{TRTSDTM}
#'     \item{ TRTSTMF }{TRTSTMF}
#'     \item{ TRTEDTM }{TRTEDTM}
#'     \item{ TRTETMF }{TRTETMF}
#'     \item{ TRTDURD }{TRTDURD}
#'     \item{ SCRFDT }{SCRFDT}
#'     \item{ EOSDT }{EOSDT}
#'     \item{ EOSSTT }{EOSSTT}
#'     \item{ FRVDT }{FRVDT}
#'     \item{ RANDDT }{RANDDT}
#'     \item{ DTHDT }{DTHDT}
#'     \item{ DTHADY }{DTHADY}
#'     \item{ LDDTHELD }{LDDTHELD}
#'     \item{ LSTALVDT }{LSTALVDT}
#'     \item{ AGEGR1 }{AGEGR1}
#'     \item{ SAFFL }{SAFFL}
#'     \item{ RACEGR1 }{RACEGR1}
#'     \item{ REGION1 }{REGION1}
#'     \item{ LDDTHGR1 }{LDDTHGR1}
#'     \item{ DTH30FL }{DTH30FL}
#'     \item{ DTHA30FL }{DTHA30FL}
#'     \item{ DTHB30FL }{DTHB30FL}
#'   }
#'
#' @source Generated from pharmaverse/admiral.
#' @references None
#'
#' @examples
#' load("adlb.rda")
"adlb"
